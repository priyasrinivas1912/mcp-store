...
// api/_utils.ts
var installedIds = /* @__PURE__ */ new Set(["github"]);
var sessionUsers = /* @__PURE__ */ new Map();
  ...
  try {
    ...
    activeServers.forEach((s) => {
      mcpServersConfig[s.id] = {
        ...
        env: s.envRequirements?.reduce((acc, env) => {
...
async function handler14(req, res) {
  ...
  try {
    ...
    const user = {
      id: `user-${Date.now()}`,
      name: String(baseName).replace(/[\._\-]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
      email,
